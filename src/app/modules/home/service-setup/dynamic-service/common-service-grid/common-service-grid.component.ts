...
@Component({
  selector: 'app-common-service-grid',
  templateUrl: './common-service-grid.component.html',
  styleUrls: ['./common-service-grid.component.scss']
})
export class CommonServiceGridComponent implements OnInit {
  ...
  columnsToDisplay: string[] = ['action', 'transId', 'employeeName', 'subservices', 'installments', 'amount', 'dated', 'paid', 'payDate'];
  ...
  ngOnInit(): void {
    this.GetServiceTypeSubServiceTypeByTenentId();
    ...
    //lblNewMemberRegistration

  }

  urlServiceData: any;
  GetServiceTypeSubServiceTypeByTenentId() {
    ...
    this.financialService.GetServiceTypeSubServiceTypeByTenentId(tenantId).subscribe((res: any) => {
      const filterValueService = res.filter((x: any) => {
        if (this.headingName.menuId == x.menuId) {
          return x;
        }
      })
      this.urlServiceData = filterValueService[0];
  ...
  loadData(pageIndex: any, val: number, val1: number, val2: number,) {
    ...
    this.financialService.GetFinancialServicesByServiceTypeAndServiceSubType(this.userParams, "", val, val1, val2, this.urlServiceData.serviceTypeId, this.urlServiceData.subServiceTypeId).subscribe((response: any) => {
      ...
      this.requiredToPrint = response.body;
  ...
  filterRecords(pageIndex: any) {
    ...
    this.financialService.setUserParams(this.userParams);
    this.financialService.GetFinancialServicesByServiceTypeAndServiceSubType(this.userParams, this.formGroup.value.searchTerm, 0, 0, 0, this.urlServiceData.serviceTypeId, this.urlServiceData.subServiceTypeId).subscribe((response: any) => {
      this.financialHeaders = JSON.parse(response.headers.get('pagination'));
      this.returnTransactionHdDto = new MatTableDataSource<ReturnTransactionHdDto>(response.body);
      this.requiredToPrint = response;
      this.returnTransactionHdDto.paginator = this.paginator;
      this.returnTransactionHdDto.sort = this.sort;
      this.isLoadingCompleted = true;
      setTimeout(() => {
  ...
  //#region
  openLoanForm() {
    this.router.navigate(['/service-setup/add-service', { serviceInformation: JSON.stringify(this.urlServiceData) }])
  }
  ...
  printData() {
    if (this.requiredToPrint.length > 0) {
      ...
      this.toastrService.warning("No data found to print", "Warning");
    }
  }
